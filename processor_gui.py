...
class TransactionProcessorGUI:
    def __init__(self, firefly_client: FireflyClient, orders: List[SimplifiedPayment], tag: str):
        self.firefly_client = firefly_client
        self.orders = orders
        self.tag = tag

    def find_matches(self, filter_text: str, exact_match: bool = True) -> List[TxMatchResult]:
        raw = self.firefly_client.fetch_transactions()
        filtered = filter_by_description(
            filter_without_category(filter_single_part(raw)),
            filter_text,
            exact_match
        )
